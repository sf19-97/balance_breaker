"""
Balance Breaker Repository Dialogs - Enhanced Version
UI dialogs for managing data repositories with robust error handling and flexible detection
"""
...
class RepositoryDialog:
    ...
    def __init__(self, parent, repo_manager, repo_type="price", on_save=None):
        self.parent = parent
    ...
    def create_dialog(self):
        ...
        self.dialog.title(f"Add {self.repo_type.title()} Repository")
        self.dialog.geometry("700x600")  # Increased size for more content
        self.dialog.minsize(700, 600)    # Set minimum size to prevent cutting off content
        ...
        ttk.Button(btn_frame, text="Save", command=self.save_repository).pack(side=tk.RIGHT, padx=5)
    ...
    def update_status(self, message, is_error=False):
        ...
        # Change status bar color based on message type
        if is_error:
            self.dialog.nametowidget('.!toplevel.!label').configure(foreground='red')
        else:
            self.dialog.nametowidget('.!toplevel.!label').configure(foreground='black')
